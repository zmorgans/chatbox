...
export default function Debate() {
  ...
  const { topic, stance, persona } = location.state || {};

  const personaPrompts = {
    academic: "You are an Academic Scholar. Use formal logic, cite research studies, and present well-structured arguments with academic rigor.",
    lawyer: "You are a Courtroom Lawyer. Cross-examine the opponent's claims, find logical inconsistencies, and demand concrete evidence.",
    philosopher: "You are a Philosopher. Question underlying assumptions, explore ethical implications, and use thought experiments.",
    journalist: "You are an Investigative Journalist. Fact-check claims, demand sources, and challenge with real-world evidence and statistics.",
    devil: "You are a Devil's Advocate. Aggressively challenge every point, find weaknesses, and push back hard on all arguments.",
    socratic: "You are a Socratic Mentor. Use probing questions to expose flaws in reasoning and guide through logical contradictions.",
  };
  ...
  const generateOpponentTurn = async (userText, round) => {
    ...
    const personaStyle = personaPrompts[persona] || personaPrompts.academic;
    
    const prompt = `${personaStyle}

You are debating and must argue the opposite stance from the user. The user is taking the ${stance} position, so you MUST take the ${aiStance} position on the topic: "${topic}".
